...
public:
   ...
   std::string getStringFromSquare() const;
   Square      getSquareFromString(const std::string &) const;

public:
   friend std::ostream& operator <<(std::ostream&, const Board&);

private:
   Piece getPieceOnSquare(const Square &) const;
   void  parseFenString(const std::string &);
   void  setBitBoard(const std::string &);
   void  setBitBoard(const std::string &, const Rank &);
   void  setCastleState(const std::string &);

private:
   std::string m_fenString;
   mutable std::unordered_map<Piece, char> m_fenPieces =\
   ...
   Square        m_passantTargetSquare;
   std::uint16_t m_castleState; /* Four bits representing castle state, same order as fen */
